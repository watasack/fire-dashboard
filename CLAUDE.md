...
- **Python エンジン**: `src/simulator.py`（参照実装・将来の API Routes 用に保持）
...
### 3. Python シミュレーターの回帰テスト（src/ 変更後）

```bash
python -m pytest tests/test_simulation_convergence.py -v
```

期待結果: `2 passed, 2 xfailed, 1 xpassed`
...
### nisa_balance <= stocks（Python エンジン・変更禁止）

`src/simulator.py` の不変条件。常に成立していること。
