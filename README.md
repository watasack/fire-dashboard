...
| カード | 表示値 | ソース |
|--------|--------|--------|
| 中央値資産（90歳時点） | ¥X,XXX万 | `median_final_assets` |
| P5残存資産（90歳時点） | ¥X,XXX万 | `percentile_5` |
| ピークからの最大下落 | -XX.X% + 金額例 | `max_drawdown_median`, `max_drawdown_p95` |
| 結果のばらつき（P10-P90） | ¥A万〜¥B万 | `percentile_10`, `percentile_90` |
...
- MC中央値: 実線で表示
- 基準線: 現在位置マーカー、FIRE達成ライン、安全マージン（500万円）
- フェーズラベル: 「蓄積期」「FIRE期」を上部に注記

**収支推移と並列表示:**
- 年次収支内訳推移（積み上げ棒グラフ）とライフイベント表を横並び（3:2比率）で配置
- 小画面ではスタック表示にフォールバック
