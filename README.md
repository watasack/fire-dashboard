...
22歳以降（独立後）
  〜69歳（アクティブ期） → empty_nest_active: 2,573,000円/年
  70〜79歳（シニア期）   → empty_nest_senior: 2,224,000円/年
  80歳〜（高齢期）       → empty_nest_elderly: 1,848,000円/年
```

カテゴリ別予算（23カテゴリ）から積み上げた値。2025年実績を反映し、保険料ゼロ・旅行統合・電気代見直し後の値。
...
3. **裁量的支出の自動分類**
   ...
   empty_nest_active（〜69歳）: 59%が裁量的（旅行・趣味等）
   empty_nest_senior（70〜79歳）: 49%が裁量的（医療費増により非裁量比率上昇）
   empty_nest_elderly（80歳〜）: 38%が裁量的（医療費・介護費が中心）
   ```
