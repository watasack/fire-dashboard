      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            {(config.withdrawalStrategy ?? 'fixed') === 'percentage' && `毎年資産残高の一定割合を取り崩します。資産が増えれば支出も増え、減れば自然に支出も減ります。最低でも月${(config.monthlyExpenses * 0.7 / 10000).toFixed(1)}万円（生活費の70%）は確保します。`}
