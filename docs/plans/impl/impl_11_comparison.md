...
**※ステータス: 実装済み**
`full_app.py`（L791-816）で「この結果をプランAとして固定」ボタンと `st.session_state['plan_a']` による保持が実装済み。
`src/visualizer.py` の `create_fire_timeline_chart` が `comparison_data` 引数（L684/769-800）でプランAの折れ線を同一グラフに重ねて描画。
