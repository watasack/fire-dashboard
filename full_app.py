...
with st.sidebar:
    st.header("👤 世帯の基本情報")
    income_h = st.number_input("🙎‍♂️ 夫の月収 (手取り/万円)", value=40, step=1, help="現在の手取り月収（ボーナス除外）。毎年2%の昇給を想定して計算します。")
    age_h = st.number_input("夫の現在の年齢", value=35, step=1)

    st.divider()
    income_w = st.number_input("🙎‍♀️ 妻の月収 (手取り/万円)", value=35, step=1, help="通常勤務時の手取り月収。育休・時短期間以外は一定として計算します（昇給なし）。")
    age_w = st.number_input("妻の現在の年齢", value=35, step=1)

    st.header("🏠 キャッシュフロー")
    expense = st.number_input("基本の月間支出 (万円)", value=28, step=1, help="住居費・食費・娯楽費など全ての合計（住宅ローンは別途10万円/月を加算して計算）")
    assets = st.number_input("現在の金融資産 (万円)", value=2000, step=100, help="現金・株式・投資信託の合計。うち30%を現金、70%を株式として計算します。NISAの既存残高は0円として扱います。")
        ...
        with tab_compare:
            ...
            if fire_row_no is not None:
                ...
                comp_data = {
                    ...
                    "フル稼働（育休なし）": [no_label, f"{fire_row_no['assets']/10000:.0f}万", "-"],
