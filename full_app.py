...
with st.sidebar:
    st.header("👤 世帯の基本情報")
    income_h = st.number_input("🙎‍♂️ 夫の月収 (手取り/万円)", value=40, min_value=1, step=1, help="現在の手取り月収（ボーナス除外）。毎年2%の昇給を想定して計算します。")
    age_h = st.number_input("夫の現在の年齢", value=35, min_value=20, max_value=70, step=1)

    st.divider()
    income_w = st.number_input("🙎‍♀️ 妻の月収 (手取り/万円)", value=35, min_value=1, step=1, help="通常勤務時の手取り月収。育休・時短期間以外は一定として計算します（昇給なし）。")
    ...
    if fire_row is not None:
        ...
        with tab_compare:
            ...
            if fire_row_no is not None:
                ...
                if diff > 0:
                    st.info(f"育休・時短プランにより、FIRE到達はフル稼働時より **{diff}年遅くなります**。")
                elif diff == 0:
                    st.info("育休・時短プランがFIRE到達年齢に影響しませんでした。")
                else:
                    st.info(f"育休・時短プランにより、FIRE到達はフル稼働時より **{abs(diff)}年早くなります**。")
